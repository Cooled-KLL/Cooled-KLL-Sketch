...
public:
    ...
    DD(int _tot_memory_in_bytes, elem_t _min_e = 0, elem_t _max_e = 1e9, double _alpha = 1e-5)
    :tot_memory_in_bytes(_tot_memory_in_bytes), min_e(_min_e), max_e(_max_e){
        int sketch_cap_ = TOTAL_ELEMENTS;
    #ifdef DICT_COLLAPSE
        int counter_number = tot_memory_in_bytes / 40; // 40: size of dict node, see dict.h
    #else
        int counter_number = tot_memory_in_bytes / sizeof(uint32_t);
    #endif
    /*
    ...
    */
        double log2gamma = std::log2(max_e - min_e + 1) / counter_number;
        double gamma = std::pow(2, log2gamma);
        double alpha = 1 - (2 / (1 + gamma));

    #ifdef DICT_COLLAPSE
        // log2gamma = std::log2(MAX_ELEM_T) / (counter_number * 100);
        // gamma = std::pow(2, log2gamma);
        // alpha = 1 - (2 / (1 + gamma));
        alpha = _alpha;
        ddsketch = new sketch::DDSketch(sketch_cap_, alpha, MAX_ELEM_T, counter_number);
    #else
...
{
    public:
    UDD(int _tot_memory_in_bytes, elem_t _min_e = 0, elem_t _max_e = 1e9, double _alpha = 1e-5)
    :DD(_tot_memory_in_bytes, _min_e, _max_e, _alpha){
        int sketch_cap_ = TOTAL_ELEMENTS;
        int counter_number = tot_memory_in_bytes / 40;
        delete ddsketch;
        ddsketch = new sketch::UDDSketch(sketch_cap_, _alpha, MAX_ELEM_T, counter_number);
    }
};