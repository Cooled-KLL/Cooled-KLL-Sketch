...
namespace sketch {
    void TDigest::dump_weighted_items(vector<weighted_item> &list){
        for (u32 i = 0; i < centroids.size(); ++i) {
            list.push_back((weighted_item){
                (elem_t)centroids[i].m_mean, (int)centroids[i].m_weight
            });
        }
    }

    void TDigest::multi_query_find_e_by_w(vector<elem_t> &result, vector<double> &w){
        vector<weighted_item> all_items;
        all_items.clear();
        dump_weighted_items(all_items);
        sort(all_items.begin(), all_items.end());
        int total_insert = TDigest::size();
    ...
    void TDigest::multi_query_find_w_by_e(vector<double> &result, vector<elem_t> &e){
        vector<weighted_item> all_items;
        all_items.clear();
        dump_weighted_items(all_items);
        sort(all_items.begin(), all_items.end());
