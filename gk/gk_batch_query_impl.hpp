...
void gk::dump_weighted_items(vector<weighted_item> &list){
    for (uint32_t i = 0; i < m_S.size(); ++i) {
        list.push_back((weighted_item){
            (elem_t)m_S[i].v, (int)m_S[i].g
        });
    }
}

void gk::multi_query_find_e_by_w(vector<elem_t> &result, vector<double> &w){
    vector<weighted_item> all_items;
    all_items.clear();
    dump_weighted_items(all_items);
    sort(all_items.begin(), all_items.end());
    int total_insert = gk::size();
...
void gk::multi_query_find_w_by_e(vector<double> &result, vector<elem_t> &e){
    vector<weighted_item> all_items;
    all_items.clear();
    dump_weighted_items(all_items);
    sort(all_items.begin(), all_items.end());
