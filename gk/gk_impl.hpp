...
class gk 
{
  public:
    ...
    elem_t quantile(double phi) const {
    ...
    struct tuple {
        ...
        tuple(elem_t v, int g, int delta) : v(v), g(g), delta(delta) {}
    };
    ...
    tuples_iterator find_insertion_index(elem_t v) {
    ...
    int size(){ return m_n; }
    int memory(){ return m_S.size() * sizeof(tuple); }
...
#include "gk_batch_query_impl.hpp"